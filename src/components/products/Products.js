...
class Products extends Component {
    ...
    AddItemsToCart = function (proItem) {
        window.location.href = `${window.location.href}/${proItem.id}` 
    };
        ...
        return this.state.loggedIn ? (
            <Box sx={{textAlign:"center",margin:'10px 10px'}}>
                ...
                <Stack 
                    ...
                    textAlign="center"
                >
                    {this.state.selectedCategory ? this.state.items.filter((item) => item.category.lower === this.state.selectedCategory.lower).map((proItem) => <ProductCard proItem={proItem} userRole = {this.state.userRole} addFunction={proItem => this.AddItemsToCart(proItem)}/> ) : this.state.items.map((proItem) => <ProductCard proItem={proItem} userRole = {this.state.userRole} addFunction={proItem => this.AddItemsToCart(proItem)}/> )}
                </Stack>
            </Box>
